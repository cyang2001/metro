...
class MetroDataset:
    ...
    def __init__(
        ...
        self.image_mode = cfg.get("image_mode")
    ...
    def _load_mat_to_df(self, path: Path) -> pd.DataFrame:
        ...
        try:
            ...
            df = df.drop(df[df['image_id'] == 120].index ) #wrong data
            df = df.drop(df[df['image_id'] == 198].index ) #wrong data
    ...
    def get_image_with_annotations(self, idx: int) -> Tuple[np.ndarray, List[Tuple[int,int,int,int,int]], int]:
        """
